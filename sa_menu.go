...
func (base *SABase) drawMenuDialogs(ui *Ui) {
	...
	if ui.Dialog_start("settings") {
		...
		ui.Comp_editbox_desc(ui.trns.DPI, 0, 4, 1, y, 1, 2, &ini.Dpi, 0, nil, "", false, false, false, true)
		y += 2

		ui.Comp_editbox_desc(ui.trns.THREADS, 0, 4, 1, y, 1, 2, &ini.Threads, 0, nil, "", false, false, false, true)
		y += 2
...
func (base *SABase) drawLauncher(app *SAApp, icon_rad float64) {
	...
	//Menu
	{
		if ui.Comp_buttonIcon(0, 0, 1, 1, InitWinMedia_url("file:apps/base/resources/logo_small.png"), 0.1, "", CdPalette_B, true, false) > 0 {
	...
	//Apps
	{
		...
		for i, app := range base.Apps {
			...
			if ui.Dialog_start(renameDialog) {
				...
				ui.Comp_editbox(0, 0, 1, 1, &base.NewAppName, 0, nil, "", false, true, false, true)
		...
		//+
		{
			...
			if ui.Dialog_start("new_app") {
				...
				ui.Comp_editbox(0, 0, 1, 1, &base.NewAppName, 0, nil, "", false, false, false, true)
